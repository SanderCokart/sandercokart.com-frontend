...
const Footer = () => {
  return (
    <footer className="space-y-4 bg-primary p-2 text-primary-foreground transition-colors sm:p-8" id="footer">
      <section>
        <FooterNavigation />
      </section>
      <section>
  ...
  return (
    <>
      ...
      <ul className="flex flex-col gap-2">
        {links.map(link => (
          <li key={link.name}>
            <Link className="font-code font-bold transition-colors hover:text-secondary" href={link.href}>
              {link.name}
            </Link>
          </li>
        ))}
...
const Copyright = () => {
  ...
  return <p className="flex justify-center text-balance text-center">© Sander Cokart {year} All rights reserved.</p>;
};
  ...
  return (
    <ul className="flex justify-center gap-4">
      {socialLinks.map(link => (
        <li key={link.name}>
          <Link aria-label={link.name} className="text-2xl transition-colors hover:text-secondary" href={link.href}>
            {link.icon}
          </Link>
        </li>
      ))}
